...
- **`aria2c` as a Daemon:** The core of the background processing is the `aria2c` download manager, which is run as a persistent daemon process. This process is started and stopped through the application's web interface, which makes calls to the `Aria2Controller`.
...
- **Daemon Process:** As mentioned above, `aria2c` is run as a long-running daemon process, not on a per-request basis.
...
- **Timeout Mitigation:** Because `aria2c` runs as a persistent background process, there is no risk of client timeouts. The application can send commands to the daemon and receive responses without being tied to the lifecycle of a single user request.
